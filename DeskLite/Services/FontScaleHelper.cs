...
public static class FontScaleHelper
{
    ...
    public static void Apply(MainWindow window, double scale)
    {
        ...
        Set(window.YearProgressPercent, 28, scale);
        Set(window.YearProgressDetail, 11, scale);
        window.YearProgressTrack.Height = FloatlyDesignTokens.ProgressBarHeight * scale;
        Set(window.CityText, 12, scale);
        window.WeatherIconImage.Width = 72 * scale;
        window.WeatherIconImage.Height = 72 * scale;
        window.SunriseIconImage.Width = 16 * scale;
        window.SunriseIconImage.Height = 16 * scale;
        window.SunsetIconImage.Width = 16 * scale;
        window.SunsetIconImage.Height = 16 * scale;
        window.TomorrowIconImage.Width = 15 * scale;
        window.TomorrowIconImage.Height = 15 * scale;
        Set(window.WeatherTempText, 48, scale);
        Set(window.WeatherDescText, 16, scale);
        Set(window.WeatherRangeText, 12, scale);
        Set(window.WeatherFeelsText, 12, scale);
        Set(window.SunriseLineText, 12, scale);
        Set(window.SunsetLineText, 12, scale);
        Set(window.TomorrowLineText, 11, scale);
        Set(window.CountdownLabel, 13, scale);
        Set(window.CountdownDays, 28, scale);
        Set(window.CountdownHint, 11, scale);
        ...
        Set(window.DailyQuoteText, 18, scale);
    ...
    private static void ApplyHuangLi(MainWindow window, double scale)
    {
        ...
        Set(window.HuangLiLunarLarge, 21, scale);
