...
> 更新时间：2026-06-18  
> 审计范围：`docs/设计图/` 三套深浅设计稿，`DeskLite/MainWindow.xaml`、`DeskLite/MainWindow.xaml.cs`、`DeskLite/SettingsWindow.xaml`、`DeskLite/SettingsWindow.xaml.cs`、`DeskLite/Services/AppConstants.cs`、`DeskLite/Services/PomodoroService.cs`，以及当前 `.codex-tmp/ui-audit/` 运行截图证据。<br>
...
- 主窗口今日待办卡片右上角 `+` 已修复为真正的新增入口：不再读取底部折叠输入框，而是直接打开项目自有待办编辑弹窗，新增后会立刻刷新主面板和待办列表窗口。
- 独立 TodoList 弹窗已按新参考图完成第一版视觉重构：窗口改为无原生标题栏的大圆角玻璃面板，补齐标题区、搜索、新建任务、全部/今天/明天/本周/已完成筛选、优先级分组任务卡、当前最重要卡、任务统计和空态入口；现有完成、星标、编辑、删除能力继续复用 `TodoStore`。
...
- 主窗口番茄钟首次开始已修复：`PomodoroService.StartOrToggle()` 在 Idle 状态下进入 Working 阶段时使用 `autoStart: true`，首次点击“开始专注”后计时器会立即运行，按钮状态会进入“暂停”，不再卡在“继续”但不倒计时的假启动状态。
...
- 最新一轮已重新生成 `release/Floatly-Setup-2.0.22.exe` 和 `release/Floatly-2.0.22-win-x64.zip`，产物时间为 `2026-06-18 13:42` 左右；安装包和 zip 已包含主窗口默认尺寸 `608 x 1080`、深色主窗口雾面密度补强、设置页 Slider 轨道拖动命中层、背景遮罩 Slider 默认启用、收入助手输入框最小尺寸修复、番茄钟主按钮圆角修复、番茄钟首次开始修复、今日待办新增入口修复、独立 TodoList 弹窗第一版重构和 v10 主面板玻璃可读性修复。
...
- 独立 TodoList 弹窗当前只是第一版视觉和交互骨架对齐：优先级分组由置顶、日期和截止日期推断，尚未新增真实优先级、标签、备注、子任务等数据字段；这些属于后续任务管理模型扩展，不能在未设计迁移规则前直接塞进存储结构。
...
- 番茄钟状态机已修复首次启动路径：Idle 状态点击开始会自动启动 DispatcherTimer；构建验证通过，服务级验证确认首次 `StartOrToggle()` 后状态为 `Working`、`IsRunning=True`、剩余时间保持工作时长初始值。
...
9. 逐项点击主窗口交互区：今日待办勾选/置顶/查看全部、番茄钟暂停/继续/重置/设置、底部主题/置顶/鼠标穿透、便签展开、黄历展开，确认不会误触发窗口拖动；番茄钟首次开始已服务级验证通过，后续重点补可见桌面会话下的完整 UI 点击截图。

## 4. 当前验收清单

必须通过的工程检查：

- `dotnet build DeskLite/DeskLite.csproj`：本轮底部工具条、周历占位、黄历“农历”标签、卡片 action 按钮、番茄钟环形轨道主题化、番茄钟遮挡修复、番茄钟首次开始修复与设置页 Slider 命中区修复后已通过，后续每轮 UI 修改后必须重跑。2026-06-18 复测构建通过，只有 NuGet 漏洞索引无法访问导致的 `NU1900` 警告。
- `PomodoroService` 服务级验证：临时 `net8.0-windows` WPF 验证程序确认首次 `StartOrToggle()` 前为 `phase=Idle, running=False, remaining=00:25:00`，首次点击路径后为 `phase=Working, running=True, remaining=00:25:00`。
