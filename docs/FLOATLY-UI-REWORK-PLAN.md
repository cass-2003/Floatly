...
- 本轮已重新生成 `release/Floatly-Setup-2.0.22.exe` 和 `release/Floatly-2.0.22-win-x64.zip`，包内包含半透明主容器、视频层按需创建、深色白色高光弱化和深色雾面密度增强后的主面板实现。
...
- 当前深色主窗口同样改为半透明圆角容器，不再依赖窗口级或媒体层背板；白色聚光感已弱化，雾面密度已增强，仍需继续按设计图控制卡片层次和稳定可读性。
