...
- 主窗口的下班倒计时、薪水、每日一句三个 `resign-2` 新增工作状态模块仍保留偏深遮罩和白字策略；深色稿接近，浅色稿还需要单独调遮罩、文字和分割线。
...
- `OffWorkCard`、`SalaryPanel`、`DailyQuoteBanner` 在 XAML 中有固定深色遮罩和白色文字，浅色主题没有完全继承 `resign-2-light.png` 的白底、浅边框、深文字语言。
...
3. 优先修正 `OffWorkCard`、`SalaryPanel`、`DailyQuoteBanner` 的浅色主题遮罩、文字、分割线和标签胶囊，使其接近 `resign-2-light.png`。
...
1. 修正主面板工作状态模块浅色主题残留：深遮罩、白字、深进度轨、暗色胶囊和每日一句深色渐变。
