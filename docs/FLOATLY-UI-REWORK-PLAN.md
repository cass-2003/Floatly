...
- 主窗口深色主题已压低 `GlassSheen` 白色叠层，并把主容器左上径向高光改为暗蓝半透明过渡，避免出现突兀白色聚光块。
- 主窗口深色主容器和模块卡片已进一步提高雾面密度，减少复杂网页/表格背景直透，避免看起来像一层灰色透明薄膜。
- 主窗口深色主容器雾面密度继续微调到约 `0xE2` 到 `0xDA`，模块卡片同步小幅加深，以贴近 `resign-2-dark.png` 的稳定暗蓝玻璃底。
...
- 本轮已重新生成 `release/Floatly-Setup-2.0.22.exe` 和 `release/Floatly-2.0.22-win-x64.zip`，包内包含半透明主容器、视频层按需创建、深色白色高光弱化、深色磨砂玻璃密度增强以及普通/图片卡片去偏光后的主面板实现。
...
- 当前深色主窗口同样改为半透明圆角容器，不再依赖窗口级或媒体层背板；白色聚光感已弱化，雾面密度已增强到更接近设计稿的暗蓝玻璃底，普通卡片和图片卡片遮罩偏光均已移除，仍需继续按设计图控制卡片层次和稳定可读性。
...
2. 截图复核主面板深色主题和不同尺寸：确认半透明主容器没有造成深色回退、浅色黑色方底残留、突兀白色聚光、背景内容过度直透或卡片方向性偏光。
