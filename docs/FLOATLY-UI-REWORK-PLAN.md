...
- 主窗口的下班倒计时、薪水、每日一句三个 `resign-2` 新增工作状态模块已完成浅色主题源码修正，但还需要运行截图确认是否完全贴近 `resign-2-light.png`。
- 还缺设置页深浅、旧主面板深浅、新主面板深浅这三套目标的逐项截图验收；目前只能按源码和设计图审计，不能宣称完全一致。
...
- 设置页已接入浅色动态资源，但还没有和 `design-3-light.png` 做逐项运行截图对比。
...
2. 运行并截图验证深色默认尺寸、浅色默认尺寸、最大化、最小尺寸四种状态。
...
- `OffWorkCard`、`SalaryPanel`、`DailyQuoteBanner` 已改为由 `ApplyWorkStateTheme()` 按深浅主题切换遮罩、文字、分割线和标签胶囊；下一步风险转为运行截图与设计稿的细节差异。
...
3. 运行截图复核 `OffWorkCard`、`SalaryPanel`、`DailyQuoteBanner` 的浅色主题遮罩、文字、分割线和标签胶囊是否接近 `resign-2-light.png`。
4. 获取主窗口默认尺寸、最大尺寸、窄尺寸、浅色主题、深色主题运行截图，并与 `resign-2` 做同屏对比。
5. 决定是否把默认窗口尺寸从 `540 x 960` 调整为更接近设计图比例的宽版尺寸，或保留当前桌面小组件密度。
6. 如果模块排序是用户可见配置，就实现真实排序；如果坚持固定视觉网格，就在设置页文案中改成“显示模块”而不是“显示顺序”。
...
1. 截图复核主面板工作状态模块浅色主题：下班倒计时、薪水、每日一句是否仍有深色残留或文字对比不足。
